...
Controls how the Salvageserver handles RW volumes that are modified/corrected
by the salvage.  Choose one of the following values:

=over 4

=item keep

Keeps the modified RW volume on the vice partition.
This is the default if the B<-orphans> argument is omitted.

=item remove

Removes (deletes) the modified RW volume, and prints a message to the
F</usr/afs/logs/SalsrvLog> file reporting which volume(s) were removed.
