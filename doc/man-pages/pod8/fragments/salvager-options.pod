...
Controls how the Salvager handles modified/corrected RW volumes.  Choose
...
Keep the modified RW volume on the vice partition.
This is the default if the B<-on-rw-error> argument is omitted.

=item remove

Removes (deletes) the modified RW volume, and prints a message to the
F</usr/afs/logs/SalvageLog> file reporting the name and volume id of
the removed volume.
