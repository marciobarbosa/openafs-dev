...
Specify C<rxgk-crypt> to use encrypted rxgk connections for server-to-server
(s2s) communication. Using rxgk s2s connections will cause the vlserver to
advertise to clients that this fileserver is rxgk-capable, allowing clients to
use rxgk credentials to access this fileserver.

Specify C<never> to use rxkad connections instead. The default is currently
C<never>.
