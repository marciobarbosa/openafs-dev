...
B<bos sa> S<<< B<-se> <I<machine name>> >>> S<<< [B<-part> <I<salvage partition>>] >>>
    ...
    S<<< [B<-or> (ignore | remove | attach)] >>> S<<< [B<-c> <I<cell name>>] >>> [B<-n>]
    S<<< [B<-on> (keep | remove)] >>>
    S<<< [B<-force>] >>>
    [B<-l>] [B<-h>]
...
=item B<-on-rw-error> (keep | remove)

Controls how the Salvager handles RW volumes that are modified/corrected
during salvage.  Choose one of the following values:

=over 4

=item keep

Leaves the modified RW volume on the vice partition, This is the
default if the B<-on-rw-error> argument is omitted.

=item remove

Removes (deletes) the modified RW volume, and prints a message to the
F</usr/afs/logs/SalvageLog> file reporting which volume(s) were removed.
