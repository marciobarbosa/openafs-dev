...
Cache all directory and file names in memory for a single volume. This option
significantly improves performance for calculating the "path" column, at the
cost of using significantly more memory. If the column "path" is not requested,
