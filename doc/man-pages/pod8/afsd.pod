...
=item B<-dynroot-whitelist>

Uses csdb as a whitelist. As a result, the cells that cannot be found
in the whitelist will not be resolved by afsdb.

Has no effect if B<-afsdb> is not enabled.
