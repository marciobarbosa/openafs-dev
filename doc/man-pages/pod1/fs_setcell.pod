...
B<fs setcell> S<<< B<-cell> <I<cell name>>+ >>>
    S<<< [B<-updaddrs> <I<primary servers>>+] >>>
    [B<-suid>] [B<-nosuid>] [B<-help>]

B<fs setce> S<<< B<-c> <I<cell name>>+ >>>
    S<<< [B<-u> <I<primary servers>>+] >>>
    [B<-s>] [B<-n>] [B<-h>]
...
The B<fs setcell> command also allows updates of the Cache Manager's
kernel-resident list of database server machines for existing cells through the
B<-updaddrs> flag. Since this option does not change the F<CellServDB> file, the
new addresses do not persist across reboots of the machine.
...
=item B<-updaddrs> <I<primary servers>>+

Specifies the fully-qualified hostnames of all AFS database server
machines for the existing cell named by the B<-cell> argument.
...
machines C<db1.example.com> and C<db2.example.com>:
   ...
   % fs setcell -cell example.com -updaddrs db1.example.com db2.example.com
