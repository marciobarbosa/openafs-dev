...
C<crypt> argument causes B<pts> to use rxgk with encryption, C<auth> causes
B<pts> to use rxgk with authentication, and C<clear> causes B<pts> to use rxgk
in clear mode.

By default, B<pts> uses rxkad credentials, falling back to unauthenticated
