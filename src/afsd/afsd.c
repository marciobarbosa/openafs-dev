...
SockProxySendProc(int a_sock, int a_idx, int a_recvpid)
{
    ...
    if (a_idx == AFS_SOCKPROXY_INIT_IDX) {
	/*
	 * this process already incremented afs_sockproxy_procs when
	 * afsd_syscall(AFS_USPC_SOCKPROXY_START) was called.
	 */
	uspc.req.usp.init = 1;
    }
...
sockproxy_thread(void *a_rock)
{
    ...
    if (recvpid == 0) {
	SockProxyReceiveProc(sock, 0);
	exit(1);
    }

    /*
     * Now fork our AFS_USPC_SOCKPROXY_SEND processes. The receiver uses idx 0,
     * so skip 0 here and start at 1.
     */
    for (idx = 1; idx < AFS_SOCKPROXY_NPROCS; idx++) {
	pid_t child;
