...
struct VolInfo {
    ...
    size_t del_vnodes_size[nVNODECLASSES];	/* number of entries in del_vnodes */
};
...
RestoreVolume(struct rx_call *call, Volume * avp, struct restoreCookie *cookie)
{
    ...
    if (vinfo.filecount != vol.filecount || vinfo.diskused != vol.diskused) {
	Log("Restore %" AFS_VOLID_FMT ": filecount %d -> %d diskused %d -> %d\n",
	    afs_printable_VolumeId_lu(V_id(vp)), vol.filecount, vinfo.filecount,
	    vol.diskused, vol.diskused);
	vol.filecount = vinfo.filecount;
	vol.diskused = vinfo.diskused;
    }
