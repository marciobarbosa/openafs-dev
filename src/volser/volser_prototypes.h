...
extern struct rx_connection *UV_Bind(afs_in_addr aserver, afs_int32 port);

extern int UV_CreateVolume(afs_in_addr aserver, afs_int32 apart, char *aname,
			   afs_uint32 * anewid);

extern int UV_DeleteVolume(afs_in_addr aserver, afs_int32 apart,
			   afs_uint32 avolid);

extern int UV_SetSecurity(struct rx_securityClass *as,
                          afs_int32 aindex);

extern int UV_ListOneVolume(afs_in_addr aserver, afs_int32 apart,
			    afs_uint32 volid, struct volintInfo **resultPtr);

extern int UV_RestoreVolume(afs_in_addr toserver, afs_int32 topart,
