...
#endif
{
    ...
    if (data == 0) {
	osi_Assert(strlcpy(mnt_stat->f_mntfromname, "AFS",
			   sizeof(mnt_stat->f_mntfromname)) < sizeof(mnt_stat->f_mntfromname));
...
#else
    osi_Assert(strlcpy(mp->mnt_stat.f_fstypename, "afs",
