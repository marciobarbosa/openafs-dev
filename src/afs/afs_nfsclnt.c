...
afs_in_addr afs_nfsclient_gethost();
...
afs_GetNfsClientPag(afs_int32 uid, afs_in_addr host)
{
 ...
 * time stamped. */
static struct nfsclientpag *
afs_FindNfsClientPag(afs_int32 uid, afs_in_addr host, afs_int32 pag)
{
    struct nfsclientpag *np;
    afs_int32 i;
...
int
afs_nfsclient_checkhost(struct nfsclientpag *np, afs_in_addr host)
{
    if (np->type != EXP_NFS)
	return 0;
    return np->host == host;
