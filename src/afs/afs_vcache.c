...
# endif
	...
	if (code) {
	    /*
	     * Since we can't leak entries that couldn't be flushed, add them to
	     * a temporary list and put all back when we get to the end of the
	     * list. This approach is not ideal and we probably need a way to be
	     * smarter about this.
	     */
	    tvc->nextfree = tmpReclaimedVCList;
	    tmpReclaimedVCList = tvc;
	}
	if (tvc->f.states & states) {
	    tvc->f.states &= ~states;
