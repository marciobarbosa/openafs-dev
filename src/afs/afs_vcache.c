...
{				/*afs_FlushVCache */
    ...
    *slept = 0;
    ...
    afs_ReclaimedVcachesRemove(avc);
...
afs_ReclaimedVcachesAdd(struct vcache *avc)
{
	...
	/*
	 * Since afs_FlushReclaimedVcaches() always drops afs_xvreclaim, don't
	 * add new entries while it is still running.
	 */
	ReleaseWriteLock(&afs_xvreclaim);
	afs_osi_Sleep(&afs_reclaim_running);
	ObtainWriteLock(&afs_xvreclaim, 1211);
    }
    QAdd(&ReclaimedVCList, &avc->reclaimq);
    ReleaseWriteLock(&afs_xvreclaim);
}

void
afs_ReclaimedVcachesRemove(struct vcache *avc)
{
    ObtainWriteLock(&afs_xvreclaim, 1212);
    if (avc->reclaimq.prev != NULL && avc->reclaimq.next != NULL) {
	/* If this entry is attached to the ReclaimedVCList, remove it. */
