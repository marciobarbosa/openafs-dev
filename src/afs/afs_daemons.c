...
afs_CheckRootVolume(void)
{
	...
	osi_Assert(strlcpy(rootVolName, "root.afs",
			   sizeof(rootVolName)) < sizeof(rootVolName));
    } else {
	osi_Assert(strlcpy(rootVolName, afs_rootVolumeName,
			   sizeof(rootVolName)) < sizeof(rootVolName));
    }
	...
	if (!tvp) {
		...
		osi_Assert(strlcpy(buf, rootVolName, sizeof(buf)) < sizeof(buf));
...
#ifdef AFS_NEW_BKG
static_inline int
do_bkg_noop(int foundAny, int n_processed)
{
...
#ifdef AFS_NEW_BKG
	if (do_bkg_noop(foundAny, n_processed)) {
	    ReleaseWriteLock(&afs_xbrs);
	    memset(uspc, 0, sizeof(*uspc));
