...
afs_CheckRootVolume(void)
{
    ...
    len = sizeof(rootVolName);
    if (*afs_rootVolumeName == 0) {
	rlen = strlcpy(rootVolName, "root.afs", len);
    } else {
	rlen = strlcpy(rootVolName, afs_rootVolumeName, len);
    }
    if (rlen >= len) {
	return ENAMETOOLONG;
    }
	...
	if (!tvp) {
	    ...
	    if ((rlen < 9) || strcmp(&rootVolName[rlen - 9], ".readonly")) {
		len = sizeof(buf);
