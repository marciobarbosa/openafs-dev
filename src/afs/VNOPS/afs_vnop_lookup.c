...
afs_DoBulkStat(struct vcache *adp, long dirCookie, struct vrequest *areqp)
{
  ...
  tagain:
	...
	rcode = afs_FSConn(&adp->f.fid, areqp, SHARED_LOCK, &cinfo);
	if (rcode == 0) {
	    tcp = cinfo.afsconn;
	    rxconn = cinfo.rxconn;
		...
		if (cinfo.fid.Fid.Volume != adp->f.fid.Fid.Volume) {
		    fidParm.AFSCBFids_val[i].Volume = cinfo.fid.Fid.Volume;
		}
	    }
	 ...
	 */
	     ...
	     (tcp, rxconn, code ? code : (&statsp[0])->errorCode,
	      &cinfo.fid, areqp, AFS_STATS_FS_RPCIDX_BULKSTATUS,
	      SHARED_LOCK, NULL));
    ...
    *avcp = tvc;		/* maybe wasn't initialized, but it is now */
	...
	if (!(flags & AFS_LOOKUP_NOEVAL))
		...
		if (tvc->mvid.target_root && (tvc->f.states & CMValid)) {
			...
			char *volname, *cpos;
			struct cell *pcell;
			int cellnum = tvc->mvid.target_root->Cell;

			pcell = afs_GetPrimaryCell(READ_LOCK);

			if (pcell && pcell->cellNum == cellnum) {
			    /*
			     * Do not add entries from the fallback cell into
			     * the cache. Those entries are not useful and can
			     * cause confusion (e.g. if the volume id used in
			     * the fallback cell is equal to the volume id of
			     * another volume in the primary cell).
			     */
