...
CheckPtConn(void)
{
    ...
    struct afsconf_dir *tdir = afsconf_Open(AFSDIR_SERVER_ETC_DIRPATH);

    if (!tdir) {
	ViceLog(0,
		("CheckPtConn: Could not open configuration directory: %s\n",
		 AFSDIR_SERVER_ETC_DIRPATH));
	return -1;
    }
    confdir_mtime = (afs_int32 *)tdir->timeRead;
    afsconf_Close(tdir);
	...
	if (!code) {
	    ...
	    osi_Assert(pthread_setspecific(viced_uclient_key, (void *)uclient)
		       == 0);
	    ViceLog(0, ("CheckPtConn: PT connections updated\n"));
	}
    }
    return code;
}
