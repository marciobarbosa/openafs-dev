	    ...
	    rxkad_level enclevel)
{
	...
	name = calloc(auser->exportedName.len + 1, sizeof(char));
	name_free = name;
	if (name == NULL) {
	    code = ENOMEM;
	    goto done;
	   ...
	   struct rx_securityClass **sc, afs_int32 *scIndex, time_t *expires)
{
	...
	*expires = 0;
	...
	if (flags & AFSCONF_SECOPTS_LOCALAUTH) {
		...
		LOCK_GLOBAL_MUTEX;
		code = GenericAuth(dir, tuser, sc, scIndex, enclevel);
		UNLOCK_GLOBAL_MUTEX;
	    }
 ...
 * RX_ID_KRB4/RX_ID_SUPERUSER users. That said, if the kind of the rx_identity
 * received as an argument is RX_ID_KRB4, the exported name must be in the
 * following format: <primary>.<instance>.
 *
