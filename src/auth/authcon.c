	    ...
	    rxkad_level enclevel)
{
	...
	name = strndup(auser->exportedName.val, auser->exportedName.len);
	name_free = name;
	if (name == NULL) {
	    code = ENOMEM;
	    goto done;
 ...
 * RX_ID_KRB4 users. That said, the kind of the rx_identity received as an
 * argument must be RX_ID_KRB4 and the exported name must be in the following
 * format: <primary>.<instance>.
 *
