 ...
 * socket operations (aka "sockproxy"). This is required on DARWIN after 10.15,
 ...
 * These run on indices 1 through AFS_SOCKPROXY_NPROCS.
 *
 ...
 *
 * When receiving packets, there is no request/replay mechanism. Instead, the
...
rx_SockProxyUpCall(int npkts, struct afs_pkt_hdr *pkts)
{
	...
	port = pkt->port;
	/* receive packet */
	p = rxi_ReceivePacket(p, SockProxySocket, host, port, 0, 0);
    }
    if (p) {
	rxi_FreePacket(p);
    }
...
SockProxyGetProc(int a_op)
{
	...
	break;
    default:
	/* Any other request shouldn't be going through the
	 * SockProxyRequest framework, so something weird is going on. */
	printf("SockProxyGetProc: proc not found.\n");
	return NULL;
    }
		   ...
		   struct afs_pkt_hdr **pkts_send)
{
	...
	for (pkt_i = 0; pkt_i < uspc->req.usp.npkts; pkt_i++) {
	    ...
	    /*rx_SockProxyUpCall(1, &pkts_recv[pkt_i]);*/
	}
	return 0;
    }
...
rxk_NewSocketHost(afs_uint32 ahost, short aport)
{
     ...
     */
    ret = rxi_Alloc(sizeof(*ret));
    AFS_GLOCK();

    osi_Assert(SockProxySocket == NULL);
    SockProxySocket = ret;
